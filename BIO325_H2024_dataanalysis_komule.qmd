...
BIO325_kolmule_otolitt_data <- read_excel("/Users/ginabroten/Documents/Master/h24/bio325/BIO325_H2024/BIO325_H2024_data_analysis/data/BIO325_kolmule_otolitt_data.xlsx", 
                                          sheet = "import til r")
...
#| 
ggplot(data = BIO325_kolmule_otolitt_data, aes(x = otolitt_l_length_mm), rm.na = TRUE, color = age) +
...
## Utlvalg av otlolitter til aldersavlesning
...
```{r}
#| label: historgram av fordelinger av aldersgrupper mot peaknummer
ggplot(BIO325_kolmule_otolitt_data, aes(x = age, fill = as.factor(peak_nr))) +
geom_histogram()
...
```{r}
#| label: log fiskelengde + log otolitgg lengde med farger som viser aldersfordeling og peaknummer
#| message: false
...
ggplot(data = BIO325_kolmule_otolitt_data_age, aes(x = log(fish_length), y = log(otolitt_l_length_mm), color = as.factor(age), shape = as.factor(peak_nr))) + 
  geom_point() +
  theme_classic() +
  labs(shape = "Peak-nr", color = "Age", y = "Log ottolith length (mm)", x = "Log fish length (m)")
```
